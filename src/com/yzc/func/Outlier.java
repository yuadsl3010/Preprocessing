...
public class Outlier {
	 ...
	 * @since 2013-8-14
	 * @throws 无
	 * @param data 数据格式为UseOutlier
	 * @return 返回经过异常点检测的数据流
	 * List<String[]>
	 */
	public List<String[]> getResult (UseOutlier data){
		n = data.n;
		k = data.k;
		cutOff = data.cutOff;
		listD = data.listD;
		List<Axis> tmpResult = findOutlier();
		for (int i = 0; i < tmpResult.size(); i++){
			//data.data.get(i)[data.intX] = String.valueOf(tmpResult.get(i).x);
			//data.data.get(i)[data.intY] = String.valueOf(tmpResult.get(i).y);
			data.data.get(tmpResult.get(i).order)[data.intX] = "异常点";
			data.data.get(tmpResult.get(i).order)[data.intY] = "异常点";
		}
		return data.data;
	}
	
	//Bay提出的基于剪枝的异常点检测算法
	private List<Axis> findOutlier(){
